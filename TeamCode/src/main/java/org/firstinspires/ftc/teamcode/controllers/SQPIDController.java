...
public class SQPIDController {
  ...
  public double calculate(double measurement, double setpoint, double targetVelocity) {
  ...
  public double calculate(double measurement) {
    ...
    double derivativeTerm = m_kd * m_errorDerivative;
    ...
    // Add kStatic if output is non-zero
    double output;
    if (Math.abs(baseOutput) < 1e-12) { // epsilonEquals check
      output = 0.0;
    } else {

...
public class BasketUnlimited extends AutoCommandBase {
  ...
  public static Pose2dHelperClass S1Basket = new Pose2dHelperClass(-60.124, -54.577, Math.toDegrees(1.1506));

  public static Pose2dHelperClass S2Basket = new Pose2dHelperClass(-64.694, -54.293, Math.toDegrees(1.621));

  // The right sample
  public static Pose2dHelperClass S1 = new Pose2dHelperClass(-59.113, -48.99884, Math.toDegrees(1.12748));

  // The middle sample
  public static Pose2dHelperClass S2 = new Pose2dHelperClass(-62.3178, -50.8168, Math.toDegrees(1.4281));

  // The left sample
  public static Pose2dHelperClass S3 = new Pose2dHelperClass(-61.953, -50.247, Math.toDegrees(1.8524));
  ...
  public Command runAutoCommand() {
    ...
    return new SequentialCommandGroup(
            ...
            new LineToLinearPathCommand(drive, S1Basket.toPose2d()).alongWith(
                    slide.aimCommand(),
                    upLiftToBasket().andThen(
            ...
            new LineToLinearPathCommand(drive, S1Basket.toPose2d()).alongWith(fastHandoff().andThen(upLiftToBasket())),
            ...
            new LineToLinearPathCommand(drive, S2Basket.toPose2d()).alongWith(fastHandoff().andThen(upLiftToBasket())),
            wait(drive, basketWaitMs),
            stowArmFromBasket().alongWith(slide.aimCommand(), slideExtendCommand.get()),

            new LineToLinearPathCommand(drive, S3.toPose2d()),
            slide.grabCommand().beforeStarting(() -> slide.setTurnServo(S3TurnPos)),
            new LineToLinearPathCommand(drive, Basket.toPose2d()).alongWith(fastHandoff().andThen(upLiftToBasket())),
            wait(drive, basketWaitMs),
            stowArmFromBasket().alongWith(slowHandoff()),

//            new InstantCommand(() -> drive.setCurrentTrajectoryMode(SampleMecanumDrive.TrajectoryMode.FAST)),

            new SplineToPathCommand(drive, splinePoint1.toPose2d(), goPickTangent),
            wait(drive, 100),
            autoSamplePickCommand(),
            new LineToLinearPathCommand(drive, Basket.toPose2d(), true).alongWith(
                    new WaitCommand(pick2Handoff).andThen(
                    slowHandoff(), upLiftToBasket())
            ),

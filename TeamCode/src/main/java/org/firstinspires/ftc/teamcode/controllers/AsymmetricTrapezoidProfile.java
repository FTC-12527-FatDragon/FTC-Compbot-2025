...
public class AsymmetricTrapezoidProfile {
    ...
    public static class Constraints {
         ...
         * @param maxDeceleration maximum deceleration (positive value)
         */
        ...
        /**
         * Constructs symmetric constraints for a TrapezoidProfile.
         *
         * @param maxVelocity maximum velocity
         * @param maxAcceleration maximum acceleration
         */
     ...
     */
    public AsymmetricTrapezoidProfile(Constraints constraints) {
    ...
    public State calculate(double t, State current, State goal) {
        ...
        // Deal with a possibly truncated motion profile with asymmetric acceleration/deceleration
        ...
        // Calculate parameters for asymmetric trapezoid
        ...
        double fullSpeedDist = fullTrapezoidDist
                - (accelerationTime * accelerationTime * m_constraints.maxAcceleration / 2.0)
                - (decelerationTime * decelerationTime * m_constraints.maxDeceleration / 2.0);

        // Handle the case where the profile never reaches full speed
        if (fullSpeedDist < 0) {
            // Calculate the peak velocity we can achieve
            double a = 1.0 / (2.0 * m_constraints.maxAcceleration) + 1.0 / (2.0 * m_constraints.maxDeceleration);
            double b = 0;
            double c = -fullTrapezoidDist;
            double peakVel = Math.sqrt(-c/a);

            if (peakVel > m_constraints.maxVelocity) {
                peakVel = m_constraints.maxVelocity;
            }

            accelerationTime = peakVel / m_constraints.maxAcceleration;
            decelerationTime = peakVel / m_constraints.maxDeceleration;
            fullSpeedDist = 0;
        }
